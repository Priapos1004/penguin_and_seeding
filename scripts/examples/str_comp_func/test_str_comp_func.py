...
# 8. added noise with random methods e.g. split
def test_noise(param: str) -> bool: 
    if param == "test123" and param.split(".")[0] == "te":
        return True
    return False

# 9. if <param> != "forbidden"
def test_neq(param: str) -> bool:
    if param != "forbidden":
        return True
    return False

# 10. if "forbidden" != <param>
def test_neq_rev(param: str) -> bool:
    if "forbidden" != param:
        return True
    return False

# 11. if not <param> == "test123"
...
if __name__ == "__main__":
    ...
    for inp in test_inputs:
        ...
        print(f"test_eq_param: {test_eq(inp)}")
        print(f"test_eq_literal_first: {test_eq_rev(inp)}")
        print(f"test_eq_literal_spaces: {test_eq_spaces(inp)}")
        print(f"test_eq_literal_no_spaces: {test_eq_no_spaces(inp)}")
        ...
        print(f"test_neq_param: {test_neq(inp)}")
        print(f"test_neq_another: {test_neq_rev(inp)}")
